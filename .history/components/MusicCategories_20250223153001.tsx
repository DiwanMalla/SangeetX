      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-6">
        {categories.map((category, index) => (
          <div key={index} className="flex justify-center">
            <Button className="w-full flex items-center gap-3 text-lg font-semibold bg-gradient-to-r from-pink-400 to-purple-500 text-white py-4 px-6 rounded-lg shadow-md hover:scale-105">
              {category.icon}
              {category.name}
            </Button>
          </div>
